...
// http2ClientOptimized implements the ClientTransport interface with HTTP2.
type http2ClientOptimized struct {
	ctx        context.Context
...
// newHTTP2ClientOptimized constructs a connected ClientTransport to addr based on HTTP2
// and starts to receive messages on it. Non-nil error returns if construction
// fails.
func newHTTP2ClientOptimized(ctx context.Context, addr TargetInfo, opts ConnectOptions) (_ ClientTransport, err error) {
	scheme := "http"
...
func (t *http2ClientOptimized) newStream(ctx context.Context, callHdr *CallHdr) *Stream {
...
// streams.
func (t *http2ClientOptimized) NewStream(ctx context.Context, callHdr *CallHdr) (_ *Stream, err error) {
	pr := &peer.Peer{
		Addr: t.remoteAddr,
	}
...
func (t *http2ClientOptimized) CloseStream(s *Stream, err error) {
	t.mu.Lock()
	if t.activeStreams == nil {
		t.mu.Unlock()
		return
...
// accessed any more.
func (t *http2ClientOptimized) Close() (err error) {
	t.mu.Lock()
	if t.state == closing {
		t.mu.Unlock()
		return
...
func (t *http2ClientOptimized) GracefulClose() error {
	t.mu.Lock()
	switch t.state {
	case unreachable:
...
func (t *http2ClientOptimized) Write(s *Stream, data []byte, opts *Options) error {
	...
	for {
		...
		// by http2ClientOptimized.Close(). No explicit CloseStream() needs to be
		// invoked.
...
func (t *http2ClientOptimized) getStream(f http2.Frame) (*Stream, bool) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
// the window.
func (t *http2ClientOptimized) adjustWindow(s *Stream, n uint32) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.state == streamDone {
		return
...
func (t *http2ClientOptimized) updateWindow(s *Stream, n uint32) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.state == streamDone {
		return
...
// estimation.
func (t *http2ClientOptimized) updateFlowControl(n uint32) {
	t.mu.Lock()
...
func (t *http2ClientOptimized) handleData(f *http2.DataFrame) {
...
func (t *http2ClientOptimized) handleRSTStream(f *http2.RSTStreamFrame) {
	s, ok := t.getStream(f)
	if !ok {
		return
	}
	...
	if !ok {
		warningf("transport: http2ClientOptimized.handleRSTStream found no mapped gRPC status for the received http2 error %v", f.ErrCode)
...
func (t *http2ClientOptimized) handleSettings(f *http2.SettingsFrame) {
	if f.IsAck() {
		return
	}
...
func (t *http2ClientOptimized) handlePing(f *http2.PingFrame) {
	if f.IsAck() {
...
func (t *http2ClientOptimized) handleGoAway(f *http2.GoAwayFrame) {
	t.mu.Lock()
	if t.state != reachable && t.state != draining {
		t.mu.Unlock()
		return
...
// the caller.
func (t *http2ClientOptimized) setGoAwayReason(f *http2.GoAwayFrame) {
...
func (t *http2ClientOptimized) GetGoAwayReason() GoAwayReason {
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.goAwayReason
}

func (t *http2ClientOptimized) handleWindowUpdate(f *http2.WindowUpdateFrame) {
...
func (t *http2ClientOptimized) operateHeaders(frame *http2.MetaHeadersFrame) {
...
func (t *http2ClientOptimized) reader() {
	...
	for {
		...
		default:
			errorf("transport: http2ClientOptimized.reader got unhandled frame type %v.", frame)
		}
	}
}

func (t *http2ClientOptimized) applySettings(ss []http2.Setting) {
	for _, s := range ss {
		switch s.ID {
...
func (t *http2ClientOptimized) controller() {
	for {
		select {
		case i := <-t.controlBuf.get():
			...
			case <-t.writableChan:
				...
				default:
					errorf("transport: http2ClientOptimized.controller got unexpected item type %v\n", i)
				}
				t.writableChan <- 0
				continue
			case <-t.shutdownChan:
...
func (t *http2ClientOptimized) keepalive() {
	p := &ping{data: [8]byte{}}
...
func (t *http2ClientOptimized) Error() <-chan struct{} {
	return t.errorChan
}

func (t *http2ClientOptimized) GoAway() <-chan struct{} {
	return t.goAway
}

func (t *http2ClientOptimized) notifyError(err error) {
	t.mu.Lock()
	...
	if t.state == reachable {
		t.state = unreachable
		close(t.errorChan)
		infof("transport: http2ClientOptimized.notifyError got notified that the client transport was broken %v.", err)
	}
	t.mu.Unlock()
}
