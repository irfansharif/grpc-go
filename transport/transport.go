...
// newContextWithStream creates a new context from ctx and attaches stream
// to it.
func newContextWithStreamOptimized(ctx context.Context, stream *StreamOptimized) context.Context {
	return context.WithValue(ctx, streamKey{}, stream)
}
...
// if it fails.
func NewServerTransport(_ string, conn net.Conn, config *ServerConfig) (ServerTransport, error) {
...
// NewClientTransportOptimized establishes the transport with the required ConnectOptions
// and returns it to the caller.
func NewClientTransportOptimized(ctx context.Context, target TargetInfo, opts ConnectOptions) (ClientTransport, error) {
	return newHTTP2ClientOptimized(ctx, target, opts)
}
...
type ServerTransport interface {
	...
	HandleStreams(handler func(*Stream), traceCtx func(context.Context, string) context.Context)
